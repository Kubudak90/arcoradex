...
/// for the Phase 1 post-upgrade testnet redeploy. Reuses existing testnet
/// MintableERC20 stables and MockChainlinkFeedV2 feeds — those addresses are
/// hardcoded below from the 2026-05-10 key-separation cutover (verified live on
/// Arc testnet).
///
/// After deploy, the script lists all 7 stables on the new Registry with their
/// per-token deviation caps + maxStaleSeconds (Phase 1 schema), then bootstraps
/// ~$100 USD-equivalent of each stable from the deployer's existing balances.
///
/// Required env:
...
contract DeployArcoraDexV2 is Script {
    ...
    uint256 constant SEED_TRYC = 4_390_000_000; // ~$100 at $0.023
    ...
    function run() external {
        ...
        console2.log("=== Deploying ArcoraDEX v2 (post-Phase-1) ===");
        ...
        ArcoraDexPool pool = new ArcoraDexPool(address(reg), SWAP_FEE_BPS, PROTOCOL_FEE_SHARE_BPS, deployer);
    ...
    function _bootstrap(ArcoraDexPool pool, address token, uint256 amount, string memory symbol) internal {
